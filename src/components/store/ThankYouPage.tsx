...
import { Product, ThankYouConfig, Order } from '../../types';
...
export default function ThankYouPage({
  ...
  const getDiscountedPrice = (price: number) => {
    if (!config.upsellDiscount) return price;
    return Math.round(price * (1 - config.upsellDiscount / 100));
  };
    ...
    <div className="min-h-screen bg-natural-white">
      <div className="max-w-3xl mx-auto px-6 py-16">
        ...
        <div className="text-center mb-10">
          ...
          <FormattedText
            text={config.message}
            className="text-text-secondary leading-relaxed max-w-xl mx-auto"
          />
        </div>
        ...
        {/* Upsell Section */}
        {config.showUpsell && upsellProducts.length > 0 && (
          <div className="mb-10">
            <div className="text-center mb-6">
              <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-warning/10 text-warning mb-4">
                <Gift className="w-4 h-4" />
                <span className="text-sm font-semibold">{config.upsellBadge || 'Special Offer'}</span>
              </div>
              <h2 className="font-display text-2xl font-semibold text-text-primary">
                {config.upsellHeading}
              </h2>
            </div>
            ...
            <div className={`grid gap-4 ${upsellProducts.length === 1 ? 'grid-cols-1 max-w-sm mx-auto' : 'grid-cols-1 sm:grid-cols-2'}`}>
              {upsellProducts.map(product => {
                ...
                const hasDiscount = config.upsellDiscount && config.upsellDiscount > 0;

                return (
                  <div
                    key={product.id}
                    className="bg-white rounded-2xl border-2 border-warning/30 overflow-hidden hover:shadow-lg transition-shadow"
                  >
                    ...
                    <div className="p-4">
                      ...
                      <h3 className="font-display text-base font-semibold text-text-primary mb-2 line-clamp-2">
                        {product.name}
                      </h3>
                      ...
                      <div className="flex items-center gap-2 mb-3">
                        ...
                        <span className="font-display text-xl font-semibold text-forest-green">
                        ...
                        {hasDiscount && (
                          <span className="px-2 py-0.5 rounded-full bg-warning/10 text-warning text-xs font-bold">
                            {config.upsellDiscount}% OFF
                          </span>
                        )}
                      </div>
                      <button
                        onClick={() => onBuyUpsell(product)}
                        className="w-full flex items-center justify-center gap-2 px-4 py-3 rounded-full bg-forest-green hover:bg-forest-green-dark text-natural-white text-sm font-medium uppercase tracking-wider transition-all"
                      >
                        Add to Order
        ...
        <div className="text-center">
          <button
            ...
            className="px-8 py-3 rounded-full border border-warm-gray text-text-secondary text-sm font-medium uppercase tracking-wider hover:border-forest-green hover:text-forest-green transition-all"
          >
            Continue Shopping
          </button>
        </div>
