  ...
  return (
    <div className="min-h-screen bg-natural-white">
      <div className="max-w-5xl mx-auto px-6 py-5">
        <button onClick={onBack} className="flex items-center gap-2 text-text-muted hover:text-forest-green text-sm font-medium uppercase tracking-elegant transition-colors">
      ...
      <section className="max-w-5xl mx-auto px-6 pb-16">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
          ...
          <div className="flex flex-col">
            <p className="text-xs font-semibold text-forest-green uppercase tracking-wider mb-2">{product.category}</p>
            <h1 className="font-display text-2xl sm:text-3xl font-semibold text-text-primary mb-3">{product.name}</h1>

            {/* Rating */}
            <div className="flex items-center gap-2 mb-5">
              <div className="flex items-center gap-0.5">
                {Array.from({ length: 5 }).map((_, i) => (
                  <Star key={i} className={`w-4 h-4 ${i < Math.floor(product.rating) ? 'text-warning fill-warning' : 'text-warm-gray'}`} />
                ))}
              </div>
              <span className="text-sm text-text-secondary">{product.rating} ({product.reviews.toLocaleString()} reviews)</span>
            </div>
            ...
            <div className="mb-6">
              {hasDiscount ? (
                <div className="flex items-center gap-3">
                  <span className="text-xl text-text-muted line-through">{cs}{product.price.toLocaleString()}</span>
                  <span className="font-display text-3xl font-semibold text-amber-700">{cs}{discountedPrice.toLocaleString()}</span>
                  {offer?.badge && <span className="px-3 py-1 rounded-full bg-amber-100 text-amber-800 text-xs font-bold">{offer.badge}</span>}
                </div>
              ) : (
                <span className="font-display text-3xl font-semibold text-text-primary">{cs}{product.price.toLocaleString()}</span>
              )}
              <span className="text-sm text-text-muted ml-2">one-time</span>
            </div>

            {/* Description */}
            <FormattedText text={product.fullDescription || product.description} className="text-text-secondary leading-relaxed mb-6 text-sm" />

            {/* Features */}
            {product.features && product.features.length > 0 && (
              <div className="mb-6">
                <h3 className="text-xs font-semibold text-text-primary uppercase tracking-wider mb-3">What's Included</h3>
                <ul className="space-y-2">
                  {product.features.map((f, i) => (
                    <li key={i} className="flex items-start gap-2">
                      <div className="w-4 h-4 rounded-full bg-forest-green/10 flex items-center justify-center shrink-0 mt-0.5"><Check className="w-2.5 h-2.5 text-forest-green" /></div>
            ...
            {/* Buy */}
            <button onClick={() => onBuy(product, offer || undefined)}
          ...
          <div className="max-w-5xl mx-auto px-6">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
              {relatedProducts.map(rp => (
                <div key={rp.id} onClick={() => onViewProduct(rp.id)} className="group bg-natural-white rounded-2xl overflow-hidden cursor-pointer hover:shadow-lg transition-all">
                  ...
                  <div className="px-3 pb-3">
                    ...
                    <div className="flex items-center justify-between mt-2">
                      <span className="font-display text-base font-semibold text-text-primary">{cs}{rp.price.toLocaleString()}</span>
                      <span className="text-[10px] text-forest-green font-medium uppercase tracking-wider flex items-center gap-1">View <ArrowRight className="w-3 h-3" /></span>
                    </div>
                  </div>
                </div>
              ))}
