...
export async function addProductToSheet(product: Omit<Product, 'id'>): Promise<Product | null> {
  ...
  if (isScriptConfigured()) {
    try {
      const r = await scriptPost({ action: 'addProduct', passcode: getStoredPasscode(), ...newProduct });
...
export async function updateProductOnSheet(productId: string, updates: Partial<Product>): Promise<boolean> {
  ...
  if (isScriptConfigured()) {
    try {
      const r = await scriptPost({ action: 'updateProduct', passcode: getStoredPasscode(), ...updated });
...
export async function deleteProductFromSheet(productId: string): Promise<boolean> {
  ...
  if (isScriptConfigured()) {
    try {
      const r = await scriptPost({ action: 'deleteProduct', passcode: getStoredPasscode(), id: productId });
      if (!r.success) return false;
    } catch { return false; }
  }
...
export function addCategory(c: Omit<Category, 'id'>): Category | null {
  ...
  const n: Category = { ...c, id: `cat-${Date.now()}` };
  all.push(n);
  saveCategories(all);
  return n;
}
...
export async function verifyPasscode(passcode: string): Promise<boolean> {
  ...
  try {
    const r = await scriptPost({ action: 'verify', passcode });
...
export function setStoredPasscode(p: string): void {
  localStorage.setItem('asqvi_passcode', p);
}
...
function saveOffersLocal(o: Offer[]): void {
...
export async function addOffer(offer: Omit<Offer, 'id'>): Promise<Offer | null> {
  if (!isLoggedIn()) return null;
  const all = getOffers();
  const n: Offer = { ...offer, id: `offer-${Date.now()}` };
  all.unshift(n);
  saveOffersLocal(all);
...
export async function deleteOffer(id: string): Promise<boolean> {
  if (!isLoggedIn()) return false;
  saveOffersLocal(getOffers().filter(o => o.id !== id));
  await syncConfigToSheet();
  return true;
}
...
export async function fetchConfigFromSheet(): Promise<{ offers: Offer[]; thankYou: ThankYouConfig }> {
  ...
  try {
    ...
    if (result.success) {
      ...
      if (result.thankYou && result.thankYou !== '{}' && result.thankYou !== '') {
        try {
          const parsed = JSON.parse(result.thankYou);
          if (parsed.rules) thankYou = parsed;
        } catch { /* */ }
      }
  ...
  } catch (e) { console.error('Fetch config from sheet failed:', e); }
  return { offers: getOffers(), thankYou: getThankYouConfig() };
}
